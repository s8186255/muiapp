define(function(require, exports, module) {
	...
	getCurrentId = function() {
		...
		if ($('li.choosed', '.contactBox').size() > 0) {
			...
			if (li) {
				...
				if (applyEdit) {
					applyEdit.evalJS(callFun+'("' + uuid + '","' + current + '","' + currentText + '","' + img + '","' + type + '")');
				}
			}
		} else {
			var applyEdit = $windowManager.getById(winId);
	...
	loadData = function() {
		$nativeUIManager.watting('正在加载审批角色...');
		$.ajax({
			type: 'POST',
			url: $common.getRestApiURL() + '/common/common/roles',
			dataType: 'json',
			data: {
