define(function(require, exports, module) {
						...
						if (!$(o).hasClass('active')) {
							...
							if (reqId) {
								...
								window.setTimeout(function() {
									...
									$windowManager.create('req_viewHead', '../req/viewHead.html?reqId=' + reqId + '&from=manage', false, true, function(show) {
										show();
									});
								}, 100);
							}
	...
	loadData = function(callback,append) {
		...
		$.ajax({
			type: 'POST',
			url: $common.getRestApiURL() + '/screen/manage/data',
			dataType: 'json',
			data: {
					...
					if (jsonData['result'] == '0') {
						...
						if (dataList && $(dataList).size() > 0) {
							...
							$(dataList).each(function(i, o) {
								...
								desc += '已被<span class="t_croose">处理</span>';
